...
**Assumptions.** CMS refreshes monthly, so **live data legitimately differs from the static sample PDF** — the sample is treated as a *layout* reference, not a data target. **Current Census is a manual input**, following the brief's data-mapping table (which specifies *Manual Input*, e.g. "112"); the baseline layout template instead hints at the CMS *Average Number of Residents per Day* field, so where the brief's prose and its layout disagree the explicit mapping table wins. (Census Capacity stays CMS-sourced, mapped to *Number of Certified Beds*.) Short-stay measures are stored as percentages (e.g. `25.6`), long-stay measures as rates per 1,000 resident-days. Star glyphs render on screen; the PDF/Word exports keep numeric ratings for print-font safety.
